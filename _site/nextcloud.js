 ...
 */
export const STORAGE = "recorder.nextcloud.v1";
...
export async function upload(cfg, filename, blob, contentType) {
...
export async function remove(cfg, filename) {
  const url = `${davBase(cfg)}/${encodeURIComponent(filename)}`;
  const res = await fetch(url, {
    method: "DELETE",
    headers: { Authorization: authHeader(cfg) },
