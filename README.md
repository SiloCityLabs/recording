...
My journey to an appless life continues with PWAs. The Play Store Google Recorder sits at **~121 MB** on my phone (app size), with about **4.27 MB** of app data and **11.83 MB** of cache even with no recordings. This PWA ships the same kind of experience in about **218 KB** (~134 KB transferred with gzip) — roughly **570× smaller**, fully offline once installed, and no store required.
...
| This PWA (all shipped files) | **~218 KB** (0.213 MB) |
| Typical transfer (gzip text + icons) | **~134 KB** |
